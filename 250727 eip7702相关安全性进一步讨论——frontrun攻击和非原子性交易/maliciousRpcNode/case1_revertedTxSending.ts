...
const calcDirectlyRevertData = async () => {
    const abi = [
        "function owner()",
        "function getName()",
    ...
    // 编码 calldata
    const calldata = iface.encodeFunctionData("directlyRevert");
...
const main = async () => {
    ...
    const tx: TransactionRequest = {
        ...
        value: 0,
        gasLimit: 80000,
        data: await calcDirectlyRevertData(),
        maxFeePerGas: toBigInt(feeData.maxFeePerGas || 42),
        maxPriorityFeePerGas: toBigInt(feeData.maxPriorityFeePerGas || 2),
    }

    // 构造并签名交易
    const signedTx = await submitterWallet.signTransaction(tx)

    // 发送交易
    const txHash = await provider.send('eth_sendRawTransaction', [signedTx]);
    console.log("standard rpc provider result, tx hash is  ", keccak256(signedTx), txHash)
}
